...
class EcsClient(object):
    ...
    def update_service(self, cluster_name, app_name, task_definition, force_deployment=False):
    ...
    def register_task_definition(self, task_definition):
        result = self.boto.register_task_definition(
            **task_definition
        )
    ...
    def list_tasks(self, cluster_name, started_by, desired_status='STOPPED'):
    ...
    def run_task(self, cluster_name, task_definition, launchtype, subnets, security_groups):
        if launchtype == LAUNCH_TYPE_FARGATE:
            if not subnets or not security_groups:
                msg = 'At least one subnet and one security ' \
